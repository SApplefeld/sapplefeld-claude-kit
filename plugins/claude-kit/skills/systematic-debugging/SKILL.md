...
- **Read the actual error.** The whole message, the whole stack, the relevant log lines — not the summary of it. Check ELEOS.ErrorLog / usp_AuditError payloads for the server-side view; the C# exception and the SQL error are often different facts.
