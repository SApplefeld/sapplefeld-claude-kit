...
**Relay mode**, interactive mode with the workstation armed. If the directory `%LOCALAPPDATA%\claude-kit\resume-relay\` exists, an AutoHotkey watcher (see `relay/` beside this skill; armed via `relay/arm-resume-relay.ps1`) performs the resume. The relay's precondition is a turn that can end cleanly: the watcher types into whatever the window holds and cannot tell an idle prompt from a blocked stop. A stop-blocking Stop hook that will not approve the boundary stop (for example a native `/goal` armed without the handoff carve-out the executing-work goal template describes) keeps the turn alive, and the typed `/resume` queues against the still-running session and fires stale at goal completion, resuming a successor that is missing everything since the boundary. Under such a hook, do not write a request; treat the boundary as relay-absent and defer. A `/kit-goal` leash approves the boundary stop deterministically once the relay handoff for this plan is written (its clause (c), enforced by the kit-goal-stop hook), and a native `/goal` carrying the carve-out is approved by its evaluator; either way the relay proceeds normally. After a successful boundary compaction, pre-flight the relay, then capture the target window and write the request atomically. Pre-flight: run `relay/preflight.ps1` (beside this skill); a relaying session cannot observe its own relay outcome (ending the turn to free the window is what lets the watcher type), so this is the one moment to catch a dead or stale watcher while you can still report it. On a nonzero exit the relay cannot fire and its one-line reason says why (not armed, watcher not running, or stale after a kit update): do not write a request; report that reason with the manual `/resume <destinationSessionId>` line and end the turn (the compaction still succeeded and the plan doc carries the state). On `OK`, capture: run `relay/capture-window.ps1` (beside this skill); it prints an `ahk_id <hwnd>` expression naming this session's own terminal window, so concurrent sessions in separate windows each resume into their own. Write the request as three or four UTF-8 lines: the destination session UUID, the destination transcript's absolute path, a single-line continue prompt naming the plan doc by its repo-relative path, e.g. `docs/plans/<plan>_spec_v1.md` (no newlines, keep it short; the watcher types it literally, and the kit-goal leash keys on that path: the hook's clause (c) matches the plan's repo-relative path in this request, then rebinds the leash to the destination session, so the successor is held from its first turn by session identity rather than by what its transcript happens to mention), and, when capture printed one, that `ahk_id` line. If capture prints nothing (a run with no host window), omit the fourth line and the watcher uses its fallback window. Write atomically: write `request.tmp` in the relay directory, then `mv` it to `request.txt`, so a poll can never read a half-written request. Make the write the turn's last tool action and keep the closing report brief; the 10-second poll can fire while the report is still rendering, which the live-fire test covers. Still report the manual `/resume` line as the fallback. The watcher validates the UUID shape, the transcript existence, and the target-window shape, types the resume and prompt, and logs to `relay.log`; a failed relay leaves the run exactly where manual mode would (the request lands in `failed\`, and the manual line stands). Targeting is per window, not per tab: two sessions sharing one terminal window as tabs both resolve to that window, so run at most one relayed session per terminal window.
