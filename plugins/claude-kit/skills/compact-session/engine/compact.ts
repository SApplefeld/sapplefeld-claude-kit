...
function parseSummaries(responseText: string, expectedCount: number): string[] {
  ...
  // Pair each echoed <user> block with the <assistant> block that follows it
  // and take exactly the first expectedCount pairs. Models regularly append
  // one unrequested summary for the trailing next-turn <user> anchor; a
  // count-only check rejects those otherwise-correct responses, while
  // positional pairing ignores the extra and still fails loudly on a true
  // miss or a structure the template did not ask for.
  const segments = [
