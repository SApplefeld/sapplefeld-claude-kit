...
- **Implementation defaults to dispatch; the main thread is the most expensive place to write code.** The main session orchestrates: it writes specs and dispatch briefs, judges findings, reads diffs, and reviews. Tier picks the model, briefability picks the locus: a section goes to the matching implementer agent (sonnet, opus, or fable, which inherits the session model or takes the explicit `fable` override from a below-fable session) whenever a fresh-context implementer can build it from the brief plus the files alone. The main thread implements only what cannot be briefed (a design still evolving in contact with the code, an in-flight debugging chain) or is too small to be worth a brief, and it is the escalation ceiling when a dispatched section fails review twice.
