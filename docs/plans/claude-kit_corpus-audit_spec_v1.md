...
Session model: fable, required, in a dedicated session that is not the implementation queue's worker; the tier split below is decision 1 and the seat split is decision 3. Authored by the KIT: Expert seat from a design dialog with the operator, 2026-08-29. Anchors are authoring-time; re-locate every hit by content, and see Execution surface before arming.
