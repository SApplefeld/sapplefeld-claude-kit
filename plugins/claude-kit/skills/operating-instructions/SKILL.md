...
- **Implementation defaults to dispatch; the main thread is the most expensive place to write code.** The main session orchestrates: it writes specs and dispatch briefs, judges findings, reads diffs, and reviews. Sections tiered sonnet or opus in a plan go to the matching implementer agent, never inline; the main thread implements only work that genuinely needs the strongest model (novel design, security-sensitive surfaces, a spec still evolving in contact with the code) or is too small to be worth a brief.
