...
4. **A check is judged on the class its own comment claims, and every control is drawn from a shape the pattern was never handed.** Added 2026-09-08 after the class surfaced in three consecutive rounds of section 2 under three mechanisms: a pin satisfied by the prose that explained it, a scoping predicate that stopped at its first match while its comment claimed the whole property, and a guard written into the one process the platform does not read it from, beside a comment asserting the closure. All three read correct at the site and could not fire, and each was written by the party least able to see it. So state the reach the pattern actually has rather than the class it was written for, and earn every silence with a control matched on shape rather than on a string the pattern already names: a control the literals name proves the instrument runs and says nothing about what it reaches. Where the class cannot be shaped, the report is that the named members are swept and the class is not.

5. **A check that carries both a behaviour claim and a class claim splits into two instruments, because neither claim is honestly established by the other's evidence.** Added 2026-09-08 on the consult ruling that closed four rounds of the same class inside one test. A behaviour claim, that these artifacts do the right thing, rests on observing them do it, and a hand model of the runtime is a claim about the model rather than about the artifact: rounds 6, 7 and 8 were each a gap between what the model spelled and what the shell does. A class claim, that an artifact added later is covered, rests on fail-closed enrollment, since a reader that judges an unfamiliar shape reports the same green whether the class is clean or merely unspelled. Run to establish the behaviour, enroll to establish the class, and constrain the artifact's shape so that the path the run takes is the only path it has, which is what makes the behavioural evidence reach the whole class rather than one traversal of it. A shape outside the enrollment is refused rather than judged, and the check's own comment states which of the two claims each half carries.
...
Acceptance: all three tests green, the parity test watched red against a deliberately wrong constant; every tracked cmd wrapper enrolled with a recipe, held to the shape allowlist, and passing all five run assertions under cmd.exe with a decoy of its interpreter planted, its guard's presence asserted on every platform for the two tracked wrappers and on Windows alone for the generated one, whose text exists only once its PowerShell generator has run, and its tracked `eol=crlf` attribute read through one token predicate on both surfaces; each predicate watched red on a shape withheld from its own literals, the allowlist's controls including accepted-line shapes rather than only refused ones, since an allowlist refuses everything outside its list by construction and a refusal-side control alone says nothing about its reach; delta named.
